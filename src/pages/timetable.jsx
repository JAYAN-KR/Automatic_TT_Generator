                    ...
                    if (teacherTimetables[tName]) {
                        ...
                        if (typeof existing === 'object' && existing && existing.subject === (slot.fullSubject || slot.subject)) {
                            ...
                            if (!classes.includes(cn)) {
                                ...
                                // No longer incrementing load metrics here because clubbed classes share the same period
                            }
                        } else {
                            teacherTimetables[tName][day][p] = {
                                className: cn,
...
Teachers can now see their timetable in the AutoSubs app.`, 'success');
    ...
    const handleCreateStreamSpecific = async (stream, baseTT = null) => {
        ...
        try {
            ...
            // support clubbed classes by splitting on '/'
            const classNames = stream.className.split('/').map(c => c.trim()).filter(Boolean);
            // Level determination based on first section (all sections should share level)
            const isMiddle = parseInt(classNames[0]) < 11;
            ...
            const streamFree = (d, p) => {
                const classFree = classNames.every(cn => (tt.classTimetables[cn]?.[d]?.[p]?.subject ?? '') === '');
                const teachersFree = stream.subjects.every(s => tFree(s.teacher, d, p));

                // Lab conflict check (Global) across all clubbed classes
            ...
            for (let i = 0; i < stream.periods; i++) {
                ...
                if (pick) {
                    ...
                    stream.subjects.forEach(s => {
                        ...
                        if (!trimmedT) return;
                        // determine if any of the clubbed classes will treat this as a lab
                        let isLab = false;
                        classNames.forEach(cn => {
                            const labStatusData = {
                                className: cn,
                                        ...
                                        return rows.map(row => {
                                            ...
                                            return (
                                                <tr key={row.id} style={{ borderBottom: '1px solid #1e293b' }}>
                                                    <td colSpan={4} style={{ padding: '0.4rem 0.6rem' }}>
                                                        ...
                                                        <div style={{
                                                            ...
                                                            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                                                ...
                                                                >
                                                                    {editingTeacherId === row.id ? '✕ Close Editor' : '🔗 Edit / Club Classes'}
                                                                </button>
                                                        ...
                                                        {editingTeacherId === row.id && (
                                                            <div className="dropdown-editor-container" style={{
                                                                ...
                                                                <TeacherGroupEditor
                                                                    row={row}
                                                                    subjects={subjects}
                                                                    classOptions={CLASSROOMS}
                                                                    onUpdateGroup={updateAllotmentGroup}
                                                                    onDeleteGroup={deleteAllotmentGroup}
                                                                    onAddGroup={addAllotmentGroup}
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                                    {gradeClassList.map(cls => {
                                        const renderCell = (dayKey, periodKey) => {
                                            ...
                                            // Updated to use ClassTTCell component
                                            return (
                                                <ClassTTCell
                                                    cell={cell}
                                                    currentClass={cls}
                                        ...
                                        return (
                                            <div key={cls} id={`class-card-${cls}`} style={{ background: '#ffffff', borderRadius: '1rem', padding: '1.5rem 2rem', border: '1px solid #000000' }}>
                                                ...
                                                {/* Added Footnote for Clubbed Classes */}
                                                <ClassTTFooter
                                                    academicYear={academicYear}
                                                    combinedSubjects={getCombinedSubjectsForClass(generatedTimetable?.classTimetables?.[cls], SUBJECT_ABBR, cls)}
                                                />
                                            </div>
                                        );
                                    })}
